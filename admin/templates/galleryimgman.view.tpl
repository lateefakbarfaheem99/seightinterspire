...
<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			if (hasReqestedFlashVersion) {
				swfu = new SWFUpload({
					// Backend Settings
					upload_url: "{{ AppPath|safe }}/admin/remote.php?remoteSection=galleryimagemanager&w=uploadimage&gal_name={{ GalleryNameUrl|safe }}",	// Relative to the SWF file or absolute
				...
				$('#btnUpload').click(function () {
					var dialog = new MultiUploadDialog({
						action: 'remote.php?remoteSection=galleryimagemanager&w=uploadimage&gal_name={{ GalleryNameUrl|safe }}&unique=' + global_randNum,
					...
					$(dialog).bind('uploadsuccess', function(evt, result){
						if (result.Filedata.duplicate) {
							...
							// success!
                            AdminImageManager.AddImage(result.Filedata.name, '../seightgallery/' + result.Filedata.name, result.Filedata.thumb, result.Filedata.filesize, result.Filedata.width, result.Filedata.height, result.Filedata.dimensions,  result.Filedata.id);
						}
					});
			...
			$('#deleteButton').bind('click', function(){
				...
				if(confirm('{% lang 'imageManagerConfirmDeleteSelectedImages' %}')) {
					...
					$.post('remote.php?remoteSection=galleryimagemanager&w=delete&gal_name={{ GalleryNameUrl|safe }}', sendPOST,
							function(json){
...
var AdminImageManager = {
	...
	GetImageRow: function() {
		...
		row += '  <input class="TemplateHeading inPlaceImageBoxDefault" id="%%image_id%%_name" value="%%image_name%%" disabled="disabled" /><br />';
		...
		row += '      <img src=\'%%image_thumburl%%\' style=" border: solid 1px #CACACA;"  id="%%image_id%%_image" width="%%image_width%%" height="%%image_height%%" title="{% lang 'imageManagerClickToViewFullSize' %}" />';
		row += '    </a>';
		row += '  </div>';
		row += '%%image_zoomicon%%';
		row += ' <a href=\'%%image_url%%\' id="%%image_id%%_urlfull" target="_blank">%%viewcaption%%</a>';
		row += ' | <a id="%%image_id%%_download" href="index.php?ToDo=downloadGalleryImage&gal_name={{ GalleryNameUrl|safe }}&image=%%image_realname%%">{% lang 'imageManagerDownload' %}</a> ';
        //row += '%%image_edit%%<br />';
		row += '  {%lang 'imageManagerSize' %}: %%image_size%% <br />';
	...
	AddImage: function(name, url, thumburl, size, displaywidth, displayheight, dimensions, id){
        ...
        //html = html.replace(/%%image_edit%%/g, ' | <a href="#" id="%%image_id%%_edit">{% lang 'Edit' %}</a>');    
        html = html.replace(/%%image_dimensions%%/g, '{%lang 'imageManagerDimensions' %}: '+dimensions);
        ...
        html = html.replace(/%%viewcaption%%/g, '{% lang 'imageManagerShowFullImage'%}');
        html = html.replace(/%%delcaption%%/g, '{% lang 'imageManagerDelete' %}');
...
function saveEditName(field){
	...
	if (newVal == '') {
		...
		$.post('remote.php?remoteSection=galleryimagemanager&w=rename&gal_name={{ GalleryNameUrl|safe }}', 'fromName=' + escape($('#' + id + '_realname').val()) + '&toName=' + escape(newVal),
			function(json){
				var result = $.evalJSON(json);
				if(result.success){
...
function ChangeImageManagerPaging(object, pagenumber) {
	...
	window.location = 'index.php?ToDo=uploadgalleryimages&page=' + pagenumber + '&id={{ GalleryID|safe }}&perpage='+ pagingamount;
}
...
function ChangeImageManagerSorting(object, pagenumber) {
	...
	window.location = 'index.php?ToDo=uploadgalleryimages&page=' + pagenumber + '&id={{ GalleryID|safe }}&sortby='+ sortby;
}
...
<table class="OuterPanel" cellpadding="0" cellspacing="0">
	<tr>
		<td class="Heading1">{% lang 'ManageGalleryImages' %}</td>
	</tr>
	<tr>
		<td class="Intro">

		<p>{% lang 'ManageGalleryImagesIntro' %}</p>
		...
		<p>
			...
			<input id="btnUpload" type="button" class="SmallButton" value="{% lang 'GalleryImageManagerUploadImages' %}" style="width: 130px;" />&nbsp;<input id="deleteButton" type="button" value="{% lang 'imageManagerDeleteSelected' %}"  class="SmallButton"  style="display: " />&nbsp;
            <input id="btnBack" type="button" class="SmallButton" value="{% lang 'GalleryImageManagerBack' %}" style="width: 150px;" onclick="document.location.href='index.php?ToDo=manageGalleryImages'"/>&nbsp;
			<input type="button" name="OpenAddon" value="Go To Gallery Editor Addon" style="cursor:pointer" onclick="window.open('?ToDo=runAddon&addon=addon_galleryeditorbywas33m')" /><br /><br />
		</p>
</td>
</tr><tr><td>
