var CategoryManager = {
	...
	*/
	...
	toggleCategorySitemap: function(categoryId) {
		// determine if this category is visible
		var visible = 1;
		var catRow = CategoryManager.getCategoryRow(categoryId);
		var toggleLink = $('#CatSitemap_' + categoryId);
		var toggleImg = toggleLink.find('img');
		if (toggleImg.attr('src') == 'images/tick.gif') {
			visible = 0;
		}
		...
		if(confirm(confirmMessage)) {
			...
			$.post("custom.php",{ 	customSection	:	'categories',
									Action			:	'toggleCategorySitemap',
									catId			:	categoryId,
									visible			:	visible
				...
				$("#CatSitemap_" + categoryId).attr('title', newTitle).find('img').attr('src', newImg);
