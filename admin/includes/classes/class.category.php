        ...
        */
        ...
        public function _BuildCategoryList($parentid = 0)
        {
            ...
            $categories = $this->nestedset->getTree(
                array('categoryid', 'catname', 'catvisible', 'catsitemap' , 'catnsetleft', 'catnsetright', 'cataltcategoriescache'),
                $parentid,
                $depth
            );
        ...
        private function CreateCategory($MsgDesc = "", $MsgStatus = "", $IsError = false)
        {
            ...
            $GLOBALS['WYSIWYG1'] = GetClass('ISC_ADMIN_EDITOR')->GetWysiwygEditor($wysiwygOptions);
        ...
        private function _GetCatData($DataSource, $CatId = 0)
        {
            ...
            if ($DataSource == ISC_SOURCE_FORM) {
                ...
                $arrCat['passwordprotect'] = ($_POST['passwordprotect'] == "ON" ) ? 1 : 0;
                $arrCat['password'] = $_POST['password'];
                $arrCat['repassword'] = $_POST['repassword'];
                $arrCat['catcontent'] = $_POST['catcontent'];
        ...
        public function _CommitCategory($catID = 0, $updateCache = true)
        {
            ...
            $_POST['catpagename'] = MakeURLSafe($_POST['catname']);
            
            $_POST['catimagefile'] = $catImage;

            $_POST['passwordprotect'] = ($_POST['passwordprotect'] == "ON") ? 1:0;
            
            if (!$_POST['passwordprotect']) {
                $_POST['password'] = '';
                $_POST['repassword'] = '';
            } else {
            ...
            if (!isId($catID)) {
                ...
                if(empty($this->categoryAPI->error)) {
                    ...
                    $parentDate = array();
                    $ids = $nestedSet->getParentPath(array('categoryid'), $catID);
                    $parentCategoryID = array_pop($ids);    // get the current category id
                    $parentCategoryID = array_pop($ids);    // get the parent category id
                    if(count($parentCategoryID) > 0) {
                        $parentData = $this->_GetCatData(ISC_SOURCE_DATABASE, $parentCategoryID['categoryid']);
                        
                        $this->categoryAPI->multiUpdateField('passwordprotect', $parentData['passwordprotect'], $cat_ids);
                        $this->categoryAPI->multiUpdateField('password', $parentData['password'], $cat_ids);
                    }                
                }
            } else {
                ...
                if(empty($this->categoryAPI->error)) {
                    ...
                    $ids = array();
                    $cat_ids = array();
                    $ids = array_merge($ids, $nestedSet->getTree(array('categoryid'), $catID));

                    foreach ($ids as $key => $val) {
                        $cat_ids[$val['categoryid']] = $val;
                    }
                    
                    $this->categoryAPI->multiUpdateField('passwordprotect', $_POST['passwordprotect'], $cat_ids);
                    $this->categoryAPI->multiUpdateField('password', $_POST['password'], $cat_ids);
                }
            }
        ...
        */
        ...
        private function EditCategory()
        {
            ...
            if (isset($_GET['catId'])) {
                ...
                $GLOBALS['IsPasswordProtect'] = isc_html_escape($this->categoryAPI->passwordprotect) ? 'checked="checked"' : '';
                $GLOBALS['CategoryPageTitle'] = isc_html_escape($this->categoryAPI->catpagetitle);
                $GLOBALS['CategoryContent'] = isc_html_escape($this->categoryAPI->catcontent);
