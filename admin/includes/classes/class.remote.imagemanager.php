...
class ISC_ADMIN_REMOTE_IMAGEMANAGER extends ISC_ADMIN_REMOTE_BASE
{
	...
	private function FetchImages($path='')
	{
        ...
        $realPath = ISC_BASE_PATH.'/uploads/'.$path;
		if(!is_dir($realPath)) {
			return $images;
		}
	...
	private function GetImagesList()
	{
		...
		foreach($imageList as $k => $image) {
            ...
            echo '["'.$image.'","'.GetConfig('AppPath').'/uploads/'.$image.'"]'.$comma."\n";
		}
		echo ');';
		exit;
	}
	...
	private function UploadImage()
	{
        ...
        $_FILES['Filedata']['imagepath'] = GetConfig('AppPath').'/uploads/';
        ...
        $destination = ISC_BASE_PATH.'/uploads/'.$name;
		...
		else if(!$this->IsValidImageFile($destination) && !$this->IsValidPDFFile($destination)) {
		...
		if (!($_FILES['Filedata']['errorfile'] || $_FILES['Filedata']['duplicate'])) {
            ...
            if($this->IsValidPDFFile($destination)) {
                $_FILES['Filedata']['thumb'] = GetConfig("ShopPath").'/product_images/PDF.jpg';
                $_FILES['Filedata']['dimensions'] = '';
                $_FILES['Filedata']['width'] = '';
                $_FILES['Filedata']['height'] = '';
            } else {
                //$_FILES['Filedata']['thumb'] = 'uploaded_images/'.$name;
                $_FILES['Filedata']['thumb'] = $this->GetImageDir().'/'.$name;
    ...
    /**
     * Check if a particular PDF is valid by checking the uploaded file header.
     *
     * @param string The path to the PDF file to check.
     * @return boolean True if the PDF is valid, false if not.
     */
    private function IsValidPDFFile($fileName)
    {
        return (file_get_contents($fileName, false, null, 0, 4) === '%PDF') ? true : false;
    }
	...
	{
		$validImages = array('png', 'jpg', 'gif', 'jpeg', 'tiff', 'bmp', 'jpe', 'pdf');
