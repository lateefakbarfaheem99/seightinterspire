...
function formatProductPrice($product, $price, array $options = array())
{
    if(strtoupper(getVisitorCountryCode()) ==  'AU') {
	    ...
	    if($product['prodsaleprice'] > 0 && $product['prodsaleprice'] < $product['prodprice']) {
		    $output .= '<span class="SalePrice">'.formatPrice($actualPrice).getLang('CatalogPriceIncTaxLabel1').'</span>';
	    }
	    else {
		    $output .= formatPrice($actualPrice).getLang('CatalogPriceIncTaxLabel1');
	    }
    } else {
        $defaultOptions = array(
		    'currencyConvert' => true,
