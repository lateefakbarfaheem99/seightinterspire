...
class ISC_IMAGEDIR {
		...
		foreach($this->dirObject as $dirItem) {
			if($this->IsImageFile($dirItem)){
				...
				$this->dirItems[] = array(
                                        ...
                                        'thumburl'=>$this->GetImageDir() . '/' . (string)$dirItem->getFilename(),
            ...
            if($this->IsPDFFile($dirItem)){
                $tmp = 0;
                $this->dirItems[] = array(
                                        'id'=>md5((string)$dirItem->getFilename()),
                                        'url'=>$this->GetImageDir() . '/' . (string)$dirItem->getFilename(),
                                        'thumburl'=>GetConfig('ShopPath') . '/product_images/PDF.jpg',
                                        'name'=>(string)$dirItem->getFilename(),
                                        'size'=>(string)filesize($dirItem->getPathname()),
                                        'modified'=>(string)filemtime($dirItem->getPathname()),
                                    );
    ...
    private function IsPDFFile($fileName)
    {
        ...
        $validFiletype = array('pdf');
        foreach($validFiletype as $type){
            if(strtolower(substr($fileName, $this->neg(strlen($type))-1)) == '.' . $type){
                return true;
            }
        }
        return false;
