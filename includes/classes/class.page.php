<?php
	...
	class ISC_PAGE
	{
		...
		public function _SetPageData($PageId=0)
		{
			if((int)$PageId === 0) {
				...
				if(isset($_REQUEST['page_id'])) {
					...
					$query = sprintf("select * from [|PREFIX|]pages where pagename='%s'", $page);
				}
				else {
		...
		*/
		 ...
		 * @return string The search item result HTML on success, empty string on error
		 */
		static public function buildContentSearchResultHTML($page, $keyword)
		{
			if (!is_array($page)) {
				return "";
			}
			...
			return self::buildSearchResultHTML($remappedPage, $keyword);
		}
		 ...
		 * @return string The search item result HTML on success, empty string on error
		 */
		static public function buildSearchResultHTML($page, $keyword)
		{
			...
			if ($page["pagetype"] == 1) {
                ...
                $pos = isc_strpos(strtolower($normalContent), strtolower($keyword));
                
                if($pos)
                    $lastPos = $pos + isc_strlen($keyword);
                    
                if(isset($lastPos) && $lastPos > 196) {
                    $smallContent = isc_substr($normalContent, $pos-50);
                    $smallContent = isc_substr($smallContent, isc_strpos($smallContent, ' '));
                    $smallContent = '...' . isc_substr($smallContent, 0, 196);
                } else
				    $smallContent = isc_substr($normalContent, 0, 199);
