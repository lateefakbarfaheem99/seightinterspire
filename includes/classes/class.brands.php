	...
	class ISC_BRANDS
	{
		...
		public function _SetBrandData()
		{
			...
			if($brand) {
				...
				$query = sprintf("select * from [|PREFIX|]brands where brandpagename='%s'", $GLOBALS['ISC_CLASS_DB']->Quote($brand));
                ...
                $this->SetBrandPageName($row['brandppagename']);
        ...
        public function SetBrandPageName($BrandPageName)
        {
            $this->_brandpagename = $BrandPageName;
        }

        public function GetBrandPageName()
        {
            return $this->_brandpagename;
        }
		...
		*/
		...
		static public function buildSearchResultsHTML($keyword)
		{
			...
			foreach ($results["results"] as $brand) {
				...
				$resultHTML[] = "<a href=\"" . BrandLink($brand["brandpagename"]) . "\">" . isc_html_escape($brand["brandname"]) . "</a>";
			}
