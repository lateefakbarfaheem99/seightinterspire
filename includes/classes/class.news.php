	...
	class ISC_NEWS
	{
		...
		public function _SetPageData()
		{
			...
			if ($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
                ...
                $this->_newspagename = $row['newspagename'];
		...
		public function ShowNews()
		{
			if ($this->_newsid > 0) {
				...
				$GLOBALS['AdditionalMetaTags'] = isset($GLOBALS['AdditionalMetaTags']) ? $GLOBALS['AdditionalMetaTags'] : "";
				$GLOBALS['AdditionalMetaTags']	.=	GetClass('ISC_FACEBOOKMETATAGS')->get_thumbnails('news',$this->_newsid);
		 ...
		 */
		static public function buildContentSearchResultHTML($news, $keyword)
		{
			if (!is_array($news)) {
				return "";
			}
			...
			return self::buildSearchResultHTML($remappedNews, $keyword);
		}
		 ...
		 * @return string The search item result HTML on success, empty string on error
		 */
		static public function buildSearchResultHTML($news, $keyword)
		{
            ...
            $normalContent = str_replace('&nbsp;','',$normalContent);

            $pos = strpos(strtolower($normalContent), strtolower($keyword));
            
            if($pos)
                $lastPos = $pos + strlen($keyword);
                
            if(isset($lastPos) && $lastPos > 196) {
                $smallContent = substr($normalContent, $pos-50);
                $smallContent = substr($smallContent, strpos($smallContent, ' '));
                $smallContent = '...' . substr($smallContent, 0, 196);
            } else
