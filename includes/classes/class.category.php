<?php
    ...
    class ISC_CATEGORY
    {
        ...
        private $_catparentid = 0;

        private $_catname = "";
        private $_catpagename = "";
        private $_catdesc = "";
        private $_catcontent = "";
        private $_catsort = "";
        ...
        private $_cat_pwprotect_enabled = '';
        private $_catpassword = '';
        ...
        public function SetCatPageName($name)
        {
            $this->_catpagename = $name;
        }

        public function GetCatPageName()
        {
            return $this->_catpagename;
        }
        ...
        {
            $this->_catcontent = $content;
        }

        public function GetContent()
        {
            return $this->_catcontent;
        }
        ...
        public function SetEnablePasswordProtect() {
            $query = "
                SELECT
                    passwordprotect
                FROM
                    [|PREFIX|]categories
                WHERE
                    categoryid = '".$this->GetId()."' AND
                    catvisible = 1
            ";
            $result = $GLOBALS['ISC_CLASS_DB']->Query($query);
            $row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
            $this->_cat_pwprotect_enabled = $row['passwordprotect'];
        }
        ...
        public function SetPassword() {
            ...
            $result = $GLOBALS['ISC_CLASS_DB']->Query($query);
            $row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
            $this->_catpassword = $row['password'];
        }
        
        public function SetParentCatId() {
            $query = "
                SELECT
                    catparentid
                FROM
            ...
            $result = $GLOBALS['ISC_CLASS_DB']->Query($query);
            $row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
            $this->_catparentid = $row['catparentid'];
        }
        
        public function GetPassword() {
            return $this->_catpassword;
        }
        
        public function isEnabledPasswordProtect() {
            return $this->_cat_pwprotect_enabled;
        }
        ...
        public function GetCatImageFile() {
            return $this->_catimagefile;
        }
        public function SetCatImageFile($Path) {
            $this->_catimagefile = $Path;
            $GLOBALS['CatImageFile'] = $Path;
        }
        ...
        public function SetCategoryData()
        {
			...
			/*Edited by WAS33M*/	$path	=	GetClass("ISC_CHANGEURLBYWAS33M")->ChangeCategoryLinkToName($path);/*Edited*/
            $this->SetSort();
            ...
            for($i = 0; $i < count($arrCats); ++$i) {
                ...
                $query = "
                    ...
                    FROM
                        [|PREFIX|]categories
                    WHERE
                        catpagename = '".$GLOBALS['ISC_CLASS_DB']->quote($arrCats[$i])."' AND
            ...
            if (!empty($category)) {
                ...
                $this->SetCatPageName($category['catpagename']);
                $this->SetDesc($category['catdesc']);
                $this->SetContent($category['catcontent']);
                ...
                $this->SetEnablePasswordProtect();
                $this->SetPassword();
                $this->SetCatImageFile($category['catimagefile']);
            } else {
            ...
            // check the category has passwordprotect mode
            $this->SetCategoryAuthState();
        ...
        public function SetCategoryAuthState() {
            $this->SetPassword();
            $this->SetEnablePasswordProtect();
            $this->SetParentCatId();
            // check the category has passwordprotect mode
            ...
            //if($GLOBALS['CatTrail'][0][1] == "MyTeamShop") {   // For this category password protect mode can be set
                $isPasswordProtected = intval($this->_cat_pwprotect_enabled);
            //}
            
            $hasauthority = false;
            if($isPasswordProtected && $this->_catparentid != 0) {  // The password protect mode is enabled for current category, so password require form will be showed.
                $password = isset($_POST['CategoryPassword']) ? $_POST['CategoryPassword'] : '';

                if($password == '') {
                    $GLOBALS['CateogoryPasswordIsWrong'] = "";
                    $GLOBALS['ShowCateogoryPasswordIsWrong'] = "display:none;";
                } else {
                    $GLOBALS['CateogoryPasswordIsWrong'] = "Password is wrong!";
                    $GLOBALS['ShowCateogoryPasswordIsWrong'] = "display:block;";
                }
                ...
                $result = $GLOBALS['ISC_CLASS_DB']->Query($query);
                $row = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
                $this->_catimagefile = $row['catimagefile'];
                $GLOBALS['CatImageFile'] = $this->_catimagefile;
                
                if(!isset($GLOBALS['CatImageFile']) || $GLOBALS['CatImageFile'] == "")
                    $GLOBALS['CategoryImageURL'] = "templates/Seight/images/ProductDefault.gif";
                else
                    $GLOBALS['CategoryImageURL'] = "product_images/".$GLOBALS['CatImageFile'];
                    
                if($password != '') {
                    $password = md5($password);
                    $password_to_check = $this->_catpassword;
                    
                    if($password == $password_to_check) { // matched typed password with category password
                        $hasauthority = true;
                    }
                }
            } else {
            ...
            $cur_cat_id = $this->GetId();
            if(!isset($_SESSION['categoryhasauthority'][$cur_cat_id]) || $_SESSION['categoryhasauthority'][$cur_cat_id] === FALSE)
                $_SESSION['categoryhasauthority'][$cur_cat_id] = $hasauthority;
        }
        /**
        ...
        public function ShowCategory()
        {
            ...
            $GLOBALS['CatPageName'] = isc_html_escape($this->GetCatPageName());
            if($_SESSION['categoryhasauthority'][$GLOBALS['CatId']]) {
                $GLOBALS['CatDesc'] = $this->GetDesc();
                $GLOBALS['CatContent'] = $this->GetContent();
            }
...
//            ";
         ...
         */
        static public function buildSearchResultsHTML($keyword)
        {
