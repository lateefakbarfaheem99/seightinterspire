	...
	class ISC_PRODUCT
	{
		...
		public function _SetProductData($productid=0)
		{
			...
			if ($productid == 0) {
				...
				/*Edited by WAS33M*/	$product	=	GetClass("ISC_CHANGEURLBYWAS33M")->ChangeProductLinkToName($product);/*Edited*/
				//$product = $GLOBALS['ISC_CLASS_DB']->Quote(MakeURLNormal($product));
				$productSQL = sprintf("p.prodpagename='%s'", $product);
			}
			else {
            ...
            $this->_prodpagename = $row['prodpagename'];
            ...
            $this->_prodcatid = $row['prodcatids'];
			...
			if ($row['opengraph_use_meta_description']) {
                ...
                $this->_opengraph_description = strip_tags($row['proddesc']);
			}
			else {
		...
		*/
		public function GetCombinationDetails($combination, $customerGroupId = null)
		{
            ...
            if(!$this->IsPurchasingAllowed()) {
                if(strtoupper(getVisitorCountryCode()) ==  'AU')
                    $variationrrp = CurrencyConvertFormatPrice($rrp).getLang('CatalogPriceIncTaxLabel1');
                else
                    $variationrrp = CurrencyConvertFormatPrice($rrp).getLang('CatalogPriceIncTaxLabel', array('label' => getConfig('taxLabel')));
            } else {
                $variationrrp = '';
            }            
			$return = array(
		...
		public function ShowPage()
		{
			if ($this->_prodid > 0) {
                ...
                foreach($productCategories as $categoryId) {
                    ...
                    $category->SetCategoryAuthState();
                    
                    if(isset($_SESSION['categoryhasauthority'][$categoryId]) && $_SESSION['categoryhasauthority'][$categoryId] === true) {
                        $canAccess = true;
                    }
                ...
                if($canAccess == false) {
                    $secureCheckPage = GetClass('ISC_SECURECHECK');
                    $secureCheckPage->HandlePage();
                    exit;
                }
				...
				$GLOBALS['AdditionalMetaTags']	=	GetClass('ISC_FACEBOOKMETATAGS')->get_thumbnails('products',$this->_prodid);
		...
		*/
		...
		static public function searchForItems($searchQuery, &$totalAmount, $start=0, $limit=-1, $sortBy="")
		{
            ...
            if(!isPasswordProtected($row['prodcatids']))
		...
		static public function buildSearchResultsHTML($keyword)
		{
			...
			foreach ($results["results"] as $product) {
				...
				if ($displayMode == "List") {
                    ...
                    $desc = str_replace('&nbsp;','',strip_tags($product["proddesc"]));

                    $pos = isc_strpos(strtolower($desc), strtolower($keyword));
                    
                    if($pos)
                        $lastPos = $pos + isc_strlen($keyword);
                        
                    if(isset($lastPos) && $lastPos > 246) {
                        $smallContent = isc_substr($desc, $pos-50);
                        $smallContent = isc_substr($smallContent, isc_strpos($smallContent, ' '));
                        $smallContent = '...' . isc_substr($smallContent, 0, 246);
                    } else
                        $smallContent = isc_substr($desc, 0, 249);
                    
                    if (isc_strlen($normalContent) > 200 && isc_substr($smallContent, -1, 1) !== ".") {
                        $smallContent .= " ...";
                    }
                    $desc = $smallContent;
