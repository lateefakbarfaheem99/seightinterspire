            ...
            if (!$this->productClass->IsPurchasingAllowed()) {
                ...
                if($this->productClass->GetProductCallForPricingLabel()) {
                    if(strtoupper(getVisitorCountryCode()) ==  'AU')
                        $GLOBALS['RetailPrice'] = CurrencyConvertFormatPrice($retailPrice).getLang('CatalogPriceIncTaxLabel1');
                    else
                        $GLOBALS['RetailPrice'] = CurrencyConvertFormatPrice($retailPrice).getLang('CatalogPriceIncTaxLabel', array('label' => getConfig('taxLabel')));
                }
                else {
                    ...
                    if(strtoupper(getVisitorCountryCode()) ==  'AU')
                        $GLOBALS['RetailPrice'] = formatPrice($retailPrice).getLang('CatalogPriceIncTaxLabel1');
                    else
                        $GLOBALS['RetailPrice'] = formatPrice($retailPrice).getLang('CatalogPriceIncTaxLabel', array('label' => getConfig('taxLabel')));
                    $GLOBALS['PriceLabel'] = GetLang('YourPrice');
                    $GLOBALS['RRPCOLOR'] = "blue";
		...
		if(!getProductReviewsEnabled()) {
			...
			if ($this->productClass->GetNumReviews() == 0) {
				...
				if ($this->productClass->GetNumReviews() == 1) {
					$GLOBALS['TotalReviews'] = $this->productClass->GetNumReviews();
					$GLOBALS['ReviewLinkText'] = GetLang('ReviewLinkText1');
				} else {
					$GLOBALS['TotalReviews'] = $this->productClass->GetNumReviews();
					$GLOBALS['ReviewLinkText'] = GetLang('ReviewLinkText2');
				}
