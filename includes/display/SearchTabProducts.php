<?php
	...
	CLASS ISC_SEARCHTABPRODUCTS_PANEL extends PANEL
	{
		public function SetPanelSettings()
		{
            ...
            $searchTerms = $GLOBALS["ISC_CLASS_SEARCH"]->GetQuery();
			// Do we have any categories
			$GLOBALS["SearchResultsCategory"] = "";

			if ($GLOBALS["ISC_CLASS_SEARCH"]->GetNumResults("category") > 0) {
				$GLOBALS["SearchResultsCategory"] = ISC_CATEGORY::buildSearchResultsHTML($searchTerms['search_query']);
			}
			...
			if ($GLOBALS["ISC_CLASS_SEARCH"]->GetNumResults("brand") > 0) {
				$GLOBALS["SearchResultsBrand"] = ISC_BRANDS::buildSearchResultsHTML($searchTerms['search_query']);
			}
			...
			if ($GLOBALS["ISC_CLASS_SEARCH"]->GetNumResults("product") > 0) {
				$productSearchResults = ISC_PRODUCT::buildSearchResultsHTML($searchTerms['search_query']);
			}
