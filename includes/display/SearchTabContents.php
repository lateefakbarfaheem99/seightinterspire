			...
			if (!empty($searchResults["results"]) && is_array($searchResults["results"])) {
				foreach ($searchResults["results"] as $item) {
					if ($item["nodetype"] == "page") {
						$contentSearchResults .= ISC_PAGE::buildContentSearchResultHTML($item, $searchResults['keyword']);
					} else {
						$contentSearchResults .= ISC_NEWS::buildContentSearchResultHTML($item, $searchResults['keyword']);
					}
				}
			}
