...
class Stats_API extends API
{
	...
	*/
	function GetUnsubscribes($statids=array(), $start=0, $perpage=10, $calendar_restrictions='', $count_only=false,$order_by = 'unsubscribetime', $order_dir = 'DESC', $listid=0)
	{
		...
		$query .= $calendar_restrictions . " ORDER BY $order_by $order_dir ";
	...
	*/
	function MarkNewsletterFinished($statids=array(), $original_queuesizes=array())
	{
		...
		while ($row = $this->Db->Fetch($result)) {
			...
			$query = "UPDATE [|PREFIX|]stats_users SET queuesize={$total_sent} WHERE statid={$statid}";
	...
	function RefundFixedCredit($jobid)
	{
		...
		$result = $this->Db->Query("SELECT userid, queuesize FROM [|PREFIX|]stats_users WHERE jobid={$jobid}");
		if (!$result) {
			trigger_error(__CLASS__ . '::' . __METHOD__ . ' - Cannot get record from database', E_USER_NOTICE);
			return false;
		}

		// Job record does not exists
		if ($result === true) {
			return true;
		}
	...
	*/
	 ...
	 */
	function ReCheckUserStats(&$user, $original_queuesize=0, $new_queuesize=0, $queuetime=0, $statid=0)
	{
		...
		if (
		    !$user->hasUnlimitedMonthlyCredit()
		    && $queueSize > (int) API_USERS::creditAvailableThisMonth($userid)
                ) {
		...
		if (
		    !$user->hasUnlimitedTotalCredit()
		    && $queueSize > (int) API_USERS::creditAvailableFixed($userid)
                ) {
	 ...
	 * @return array Returns an array of status and a language variable describing why it can't be sent. This allows us to differentiate between whether it's a "maxemails" issue or a "per month" issue.
	 */
	public function CheckUserStats(User_API $user, $queueSize = 0, $queueTime = 0, $onlyCheckPerMonth = false)
	{
        // if they have no limits, then no need to do any other checks
		if ($user->hasUnlimitedCredit()) {
			return array(true, false);
