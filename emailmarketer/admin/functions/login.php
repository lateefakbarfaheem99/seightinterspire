...
class Login extends SendStudio_Functions
{
	...
	function Process()
	{
		...
		switch ($action) {
			...
			case 'login':
				...
				$rememberdetails = (IEM::requestGetPOST('rememberme', false) !== false);
				...
				if ($rememberdetails) {
					$usercookie_info = array('user' => $username, 'time' => time(), 'takemeto' => $redirect);
	...
	*/
	function ShowLoginForm($template=false, $msg=false)
	{
		...
		$userCookieInfo = IEM::requestGetCookie('IEM_CookieLogin', array());
		if (is_array($userCookieInfo) && isset($userCookieInfo['user'])) {
			$GLOBALS['ss_username'] = htmlspecialchars($userCookieInfo['user'], ENT_QUOTES, SENDSTUDIO_CHARSET);
		}
